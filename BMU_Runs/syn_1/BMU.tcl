...
set rc [catch {
  open_project {BMU.prj}
  import_device ef2_4.db -package EF2L45UG132B
  elaborate -top {ns213_bmu_cpld_top}
  export_db {BMU_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../src/NS213_BMU.adc
  optimize_rtl
  report_area -file BMU_rtl.area
  export_db {BMU_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea BMU_gate.area
  legalize_phy_inst
  export_db {BMU_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
